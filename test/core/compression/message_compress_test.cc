...
static void assert_passthrough(grpc_slice value,
...

  {
    ...
    was_compressed = grpc_msg_compress(algorithm, &input, &compressed_raw, grpc_core::GzipCompressionOptions());
  }
...
TEST(MessageCompressTest, TinyDataCompress) {
  ...
  for (int i = 0; i < GRPC_COMPRESS_ALGORITHMS_COUNT; i++) {
    ...
    ASSERT_EQ(0, grpc_msg_compress(static_cast<grpc_compression_algorithm>(i),
                                   &input, &output, grpc_core::GzipCompressionOptions()));
...
TEST(MessageCompressTest, BadDecompressionDataCrc) {
  ...
  grpc_core::ExecCtx exec_ctx;
  // compress it
  grpc_msg_compress(GRPC_COMPRESS_GZIP, &input, &corrupted, grpc_core::GzipCompressionOptions());
...
TEST(MessageCompressTest, BadDecompressionDataMissingTrailer) {
  ...
  grpc_core::ExecCtx exec_ctx;
  // compress it
  grpc_msg_compress(GRPC_COMPRESS_GZIP, &input, &decompressed, grpc_core::GzipCompressionOptions());
...
TEST(MessageCompressTest, BadCompressionAlgorithm) {
  ...
  grpc_core::ExecCtx exec_ctx;
  was_compressed =
      grpc_msg_compress(GRPC_COMPRESS_ALGORITHMS_COUNT, &input, &output, grpc_core::GzipCompressionOptions());
